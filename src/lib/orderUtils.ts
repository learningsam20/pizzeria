...
import { comboGroupLabel, groupOrderItemsIntoCombos } from './orderFormat';
...
export function formatBillRows(items: Pick<OrderItem, 'name' | 'quantity' | 'unit_price_snapshot' | 'category' | 'id'>[]) {
  const groups = groupOrderItemsIntoCombos(items as OrderItem[]);
  if (groups.length) {
    return groups.map(g => ({
      label: `Combo ${g.index}: ${comboGroupLabel(g)}`,
      amount: g.subtotal,
    }));
  }
  return items.map(i => ({
